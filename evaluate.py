...
def test_model_all(prediction, test_mask, train_mask):
    ...
    prediction = prediction + train_mask * -1000000.0
...
def test_model_batch(prediction, test_mask, train_mask):
    ...
    prediction = prediction + train_mask * -100000000.0
...
def test_all(sess, model):
     Decoder = sess.run(model.rating, {model.users: list(range(model.num_users)), model.pos_items: list(range(model.num_items))})
